...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    // Create and format  time object 
    std::tm timeinfo;
    time_t rawtime;
    time(&rawtime);
    char buffer[80];

    // Use localtime_s ather than localtime in order to protect against buffer overflow.
    localtime_s(&timeinfo, &rawtime);

    // Place the timeinfo value from the pointer into buffer
    strftime(buffer, 80, "%F", &timeinfo);

    // If the file was successfully opened,
    if (saveFile.is_open()) {

        // output a message that the file was opened successfully.
        ...
        saveFile << "Time: " << buffer << "\n";

        // key used,
        saveFile << "Key: " << key << "\n";

        // and the data included in the 'read_file()' function.
        saveFile << "Data: " << data << "\n";
